...
const InvoiceForm = () => {
  ...
  useEffect(() => {
    // Fetch the latest invoice from DB to auto-increment numbers
    window.api.listInvoices().then((invoices) => {
      if (invoices && invoices.length > 0) {
        const last = invoices[invoices.length - 1];
        setInvoiceNumber((Number(last.invoiceNumber) + 1).toString());
        setBiltyNumber((Number(last.biltyNumber) + 1).toString());
        setRefNumber((Number(last.refNumber) + 1).toString());
      }
    });
    // Settings can still be loaded from localStorage
  ...
  const handleSaveInvoice = async () => {
    ...
    await window.api.addInvoice(invoiceData);
